...
Add this section to `<!-- ADAPTER: context-file -->`. If the file already exists, append it — don't replace existing content (there may be build commands, linting config, or other project-specific instructions already there that the SDD workflow needs to read).
...
- For `[P]` (parallel) tasks: <!-- ADAPTER: parallel-tasks -->
